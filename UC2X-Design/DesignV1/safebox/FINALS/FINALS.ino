...
//SIM TRUE  internet
#define APN "internet"  //  true=internet,AIS=internet,DTAC=www.dtac.co.th,
#define USER "true"     //  true=true,AIS=ais,DTAC= ""
#define PASS "true"    //   true=true,AIS=ais,DTAC= ""
...
void loop() {
  ...
  if (int(firebase.connect()) == true) {
    apsetup = firebase.getInt("boxs/" + boxname + "/apsetup");
    micrecord = firebase.getInt("boxs/" + boxname + "/micrecord");
    cbtemp = firebase.getInt("cboxs/" + boxname + "/btemp");
    cbmotion =  firebase.getInt("boxs/" + boxname + "/cbmotion");
    setinterval = firebase.getInt("boxs/" + boxname + "/setinterval");
    Serial.println("apsetup:" + String(apsetup));
...
//    MicsRestory();
  ...
  if (runs == true && digitalRead(water1) == LOW && currentMillis2 - previousMillis2 >=  60 * setinterval) {
    Serial.println("car stop water1");
    if (int(firebase.connect()) == 1) {
      firebase.setInt("boxs/" + boxname + "/lastWater1", digitalRead(water1));
      delay(100);
      firebase.setStr("boxs/" + boxname + "/lastLatitude",  latitude);
      delay(100);
      firebase.setStr("boxs/" + boxname + "/lastLongitude", longitude);
      delay(100);
    }
    firebase.close();
  ...
  if (runs == true && digitalRead(water2) == LOW && currentMillis3 - previousMillis3 >=  60 * setinterval) {
    Serial.println("car stop water2");
    if (int(firebase.connect()) == 1) {
      firebase.setInt("boxs/" + boxname + "/lastWater2", digitalRead(water2));
      delay(100);
      firebase.setStr("boxs/" + boxname + "/lastLatitude",  latitude);
      delay(100);
      firebase.setStr("boxs/" + boxname + "/lastLongitude", longitude);
  ...
  if (runs == true && temperatureC > cbtemp && currentMillis4 - previousMillis4 >=  60 * setinterval) {
    Serial.println("car stop temperatureC");
    if (int(firebase.connect()) == 1) {
      firebase.setFloat("boxs/" + boxname + "/temperature", temperatureC);
    }
    firebase.close();
  ...
  if (runs == true && motion_sensor == 1 && currentMillis5 - previousMillis5 >=  60 * setinterval) {
    Serial.println("car stop motion_sensor");
    if (int(firebase.connect()) == 1) {
      firebase.setFloat("boxs/" + boxname + "/lastMotion", motion_sensor);
    }
    motion_sensor = 0;
    count = 0;
    firebase.close();
