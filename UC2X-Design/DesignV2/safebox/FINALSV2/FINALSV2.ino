...
void loop() {
  ...
  if (setups == 0) {
    ...
    if (int(firebase.connect()) == 1) {
    //      apsetup = firebase.getInt("boxs/" + mac_address + "/apsetup");
    //      aprestory = firebase.getInt("boxs/" + mac_address + "/aprestory");
      micrecord = firebase.getInt("boxes/" + mac_address + "/micrecord");
      ...
      Serial.println("aprestory:" + String(aprestory));
    ...
    if (runs == true && digitalRead(water1) == LOW && aroundstate == false) {
      water_sensor_1 = 1;
      aroundstate = true;
    ...
    if (runs == true && digitalRead(water2) == LOW && aroundstate == false) {
      water_sensor_2 = 1;
      aroundstate = true;
      unsigned long currentMillis3 = millis();
      if (currentMillis3 - previousMillis3 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case Water2");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setInt("boxes/" + mac_address + "/lastWater2", water_sensor_2);
          delay(100);
          firebase.setStr("boxes/" + mac_address + "/lastLatitude",  latitude);
          firebase.setStr("boxes/" + mac_address + "/lastLongitude", longitude);
          delay(100);
